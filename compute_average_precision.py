...
def compute_AP(img1, img2, kp1, kp2, max_match, H_path, method, img_save_path, imshow=False):
    if imshow:
        ...
        if method == 'L2':
            Thresh = max_list[14]
        if method == 'cos':
            Thresh = max_list[-int(max_list_len/10)]
...
def show_keypoints(kp1,kp2,img1,img2,detect_correct_match,max_match,img_save_path,H_path):
    ...
    ####按列拼接
    rows1 = img1.shape[0]
    #img3 = append_image_col(img1,img2)
    ...
    for i in range(len(kp_1)):
        ...
        ####按列拼接
        #cv2.circle(img3,(int(np.round(x2)),int(np.round(y2)+rows1)),2,(0,155,255),2)
        #if ((x1,y1) in key_points1) and ((x2,y2) in key_points2):
        ...
        if compute_corresponse_distance(kp_1[i], kp_2[i], H_path) < 16:
            ...
            #cv2.line(img3, (int(np.round(x1)),int(np.round(y1))), (int(np.round(x2)),int(np.round(y2)+rows1)), (255, 0, 0), 2, lineType=cv2.LINE_AA, shift=0)
        else:
            ...
            #cv2.line(img3, (int(np.round(x1)),int(np.round(y1))), (int(np.round(x2)),int(np.round(y2)+rows1)), (0, 0, 255), 2, lineType=cv2.LINE_AA, shift=0)
